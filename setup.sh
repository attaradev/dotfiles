...
write_optional_cask_env() {
  local entry suffix var_name tmp_file
...

  {
    ...
    for entry in "${OPTIONAL_CASK_SPECS[@]}"; do
      ...
      var_name="HOMEBREW_BUNDLE_INSTALL_${suffix}"
      echo "export $var_name=${!var_name:-0}"
    done
  } > "$tmp_file"
...
configure_optional_cask() {
  ...
  local label="$2"
  local description="$3"
  local var_name="HOMEBREW_BUNDLE_INSTALL_${suffix}"
...
for cask_spec in "${OPTIONAL_CASK_SPECS[@]}"; do
  ...
  configure_optional_cask "$cask_suffix" "$cask_label" "$cask_description"
done
