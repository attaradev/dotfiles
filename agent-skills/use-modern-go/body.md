...
If you cannot detect the Go version automatically, ask the user: "Which Go version should I target?" → [1.23] / [1.24] / [1.25] / [1.26]
...
**time package:**
- `time.Tick`: Use freely — as of Go 1.23, the garbage collector can recover unreferenced tickers even if they haven't been stopped. There is no longer any reason to prefer `NewTicker` when `Tick` will do.
...
- `new(val)` not `x := val; &x`. Go 1.26 extends `new()` to accept expressions. Type is inferred: `new(0)` → `*int`, `new("s")` → `*string`, `new(T{})` → `*T`. DO NOT use `x := val; &x`. DO NOT use redundant casts like `new(int(0))` — just write `new(0)`.
...
- `errors.AsType[T](err)` not `errors.As(err, &target)`. ALWAYS use `errors.AsType` when checking if an error matches a specific type.
