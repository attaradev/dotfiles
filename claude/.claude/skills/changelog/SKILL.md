...
- Commits since last tag: !`git log $(git describe --tags --abbrev=0 2>/dev/null || echo "")..HEAD --oneline 2>/dev/null || git log --oneline -30`
- Commit details since last tag: !`git log $(git describe --tags --abbrev=0 2>/dev/null || echo "")..HEAD --format="%s%n%b" 2>/dev/null | head -150 || true`
