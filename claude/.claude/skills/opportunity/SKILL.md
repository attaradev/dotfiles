...
Be honest about unknowns. Quantify where possible; flag where you cannot. Explicitly state the evidence that would change the recommendation. Separate market risk (is the problem real?), feasibility risk (can we build it?), and viability risk (can we capture value?). A good assessment makes the investment case OR argues against it.
